            ...
            <div class="table-responsive">
                <table class="table table-borderless table-striped align-middle">
                    ...
                    <tbody>
                        ...
                        @endif
                        @foreach ($portfolio as $result => $key)
                            <tr>
                                <td class="text-center">
                                    <form action="{{route('portfolio.destroy', Crypt::encrypt($key->id))}}" method="POST">
                                        @csrf
                                        @method('delete')
                            ...
                            <div class="modal modal-blur fade" id="modal-show_{{$key->slug}}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered" role="document">
                                    <div class="modal-content">
                                        ...
                                        <form action="{{route('portfolio.update', Crypt::encrypt($key->id))}}" method="POST" enctype="multipart/form-data">
                                            @csrf
                                            @method('patch')
                                                ...
                                                <div class="mb-3">
                                                    <label class="form-label">
                                                        <div class="d-flex justify-content-between">
                                                            <span>Image</span>
                                                            @if ($key->image !== null)
                                                                <a href="{{asset('uploads/portfolio/'.$key->image)}}" target="_blank">View image</a>
                                                            @endif
                                                        </div>
                                                    </label>
